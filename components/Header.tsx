...
const Header: React.FC<HeaderProps> = ({
  ...
  const getViewTitle = () => {
    switch (currentView) {
      case View.DASHBOARD: return t('sidebar.items.dashboard');
      case View.USERS: return t('sidebar.items.users');
      case View.ORDER_CONFIRMATION: return t('sidebar.items.order_confirmation');
      case View.ORDER_TRACKING: return t('sidebar.items.order_tracking');
      case View.ORDER_ABANDONED: return t('sidebar.items.abandoned_orders');
      case View.SALARIES: return t('sidebar.items.salaries');
      case View.FINANCES: return t('sidebar.items.finances');
      case View.FINANCIAL_STATS: return t('sidebar.items.financial_stats');
      case View.INTEGRATION_SETTINGS: return t('sidebar.items.google_sheets');
      case View.INVENTORY: return t('sidebar.items.inventory');
      case View.SHIPPING_CARRIERS: return t('sidebar.items.shipping_carriers');
      case View.SHIPPING_PRICING: return t('sidebar.items.shipping_pricing');
      case View.STORE_LINKING: return t('sidebar.items.store_linking');
      // case View.API_DOCS: return t('sidebar.items.api_docs');
      case View.SUBSCRIPTIONS: return t('sidebar.items.subscriptions');
      default: return 'Wilo';
    }
  };

  const today = new Date().toLocaleDateString(i18n.language === 'ar' ? 'ar' : (i18n.language === 'fr' ? 'fr-FR' : 'en-US'), { weekday: 'long', day: 'numeric', month: 'long' });
    ...
    <header className="h-16 bg-white border-b border-slate-200/50 flex items-center justify-between px-4 md:px-6 z-30 transition-all duration-300 shrink-0">
      ...
      <div className="flex-1 max-w-sm mx-4 hidden lg:block">
        <div className="relative group">
          ...
          <input
            ...
            className={`w-full ${i18n.dir() === 'rtl' ? 'pr-10 pl-10' : 'pl-10 pr-10'} py-2 bg-slate-100/50 border-transparent border focus:border-indigo-500/30 focus:bg-white rounded-lg focus:outline-none transition-all text-xs font-medium placeholder:text-slate-400 text-right`}
          />
          {searchQuery && (
            <button
              onClick={() => {
          ...
          {showSpotlight && searchQuery.length > 1 && (
            <div className="absolute top-full mt-2 w-full sm:w-[500px] left-1/2 -translate-x-1/2 bg-white border border-slate-100 rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              <div className="p-3 border-b border-slate-50 bg-slate-50/50 flex justify-between items-center">
                <span className="text-[10px] font-black text-slate-400 uppercase">{t('common.search_results')}</span>
                ...
                {searchData?.allOrder?.data?.map((order: any) => (
                  ...
                  >
                    ...
                    <div className="flex-1 overflow-hidden">
                      <h5 className="text-[11px] font-black text-slate-700 truncate group-hover:text-indigo-600">{order.fullName || t('common.no_name')}</h5>
                    ...
                    <div className="flex flex-col items-end shrink-0 gap-1">
                      ...
                      >
                        {order.status?.nameAR || t('common.not_specified')}
                      </span>
                      <span className="text-[10px] font-black text-slate-800">{order.totalPrice?.toLocaleString()} {t('common.currency')}</span>
                    </div>
                  </button>
                ))}
              </div>
      ...
      <div className="flex items-center gap-2 md:gap-3">
        ...
        {currentUser.role !== 'confirmed' && (() => {
          ...
          let btnText = t('header.subscribe');
          ...
          return (
            ...
            >
              {btnIcon}
              <span className="hidden sm:inline">{currentView === View.SUBSCRIPTIONS ? t('header.subscribe') : btnText}</span>
            </button>
          );
        })()}
        ...
        <div className="relative">
          ...
          {isUserMenuOpen && (
            <>
              ...
              <div className={`absolute ${i18n.dir() === 'rtl' ? 'left-0' : 'right-0'} top-full mt-2 w-64 bg-white rounded-xl shadow-xl border border-slate-100 z-50 overflow-hidden animate-in slide-in-from-top-2 fade-in duration-200 origin-top-${i18n.dir() === 'rtl' ? 'left' : 'right'}`}>
                <div className="p-4 bg-slate-50/50 border-b border-slate-100">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{t('header.logged_in_as')}</p>
              ...
              <div className="relative bg-white w-full max-w-md rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-10 duration-300 max-h-[90vh] flex flex-col">
                ...
                <div className="p-6 md:p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center shrink-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest leading-none">{t('header.profile_settings')}</h4>
                      <p className="text-[9px] text-slate-400 font-bold uppercase mt-1 tracking-tighter">{t('header.update_profile_desc')}</p>
                    </div>
                  </div>
                ...
                <div className="p-6 md:p-10 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">{t('header.full_name')}</label>
                    <div className="relative">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">{t('header.email')}</label>
                    <div className="relative">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">{t('header.phone')}</label>
                    <div className="relative">
                  ...
                  <div className="pt-4 grid grid-cols-2 gap-3 sm:gap-4">
                    ...
                    <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex flex-col items-center gap-2 text-center">
                      ...
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t('header.joined_at')}</span>
                ...
                <div className="p-6 md:p-8 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row gap-3 sm:gap-4 shrink-0 justify-end">
                  ...
                  >
                    <Save className="w-4 h-4" /> {t('header.save_changes')}
                  </button>
                </div>
              </div>
            </div>,
